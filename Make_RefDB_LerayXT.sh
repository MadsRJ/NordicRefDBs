...
if [ ! -f "$output_dir/FilteredRefs_CRABS.txt" ]; then
	...
	mv "$output_dir"/FilteredRefs2_CRABS.txt "$output_dir"/FilteredRefs_CRABS1.txt
	cat "$output_dir"/FilteredRefs_CRABS1.txt | sort -k7 > "$output_dir"/tmp.txt
	mv "$output_dir"/tmp.txt "$output_dir"/FilteredRefs_CRABS.txt
	rm "$output_dir"/FilteredRefs_CRABS1.txt
...
if [ ! -f "$output_dir/BLAST_TAX_COI/BLAST_TAX_COI.nsq" ]; then
    current_datetime=$(date +"%Y-%m-%d_%H-%M-%S")
    print_yellow "[$current_datetime] Exporting filtered database to blast-tax format ..."
	mkdir -p "$output_dir"/BLAST_TAX_COI/
    crabs \
    --export \
